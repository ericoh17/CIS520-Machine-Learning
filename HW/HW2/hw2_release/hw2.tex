...
In general, we observe that as $N$ increases, the training error decreases as we would expect. For KNN in both the original and noisy datasets, the test error is lower than that of the training error, suggesting that there is not much overfitting. For Kernel Regression, the noisy dataset results in higher test error than training error, suggesting that we might be overfitting to noise. 
